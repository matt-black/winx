...
def test_axes_same_shape0():
    y = generic_filter(
        jnp.asarray(_b), identity, 3, None, "constant", 0, 0, axes=(1, 2)
    )
...
def test_axes_same_shape1():
    z = generic_filter(
        jnp.asarray(_c), identity, 3, None, "constant", 0, 0, axes=(2, 3)
    )
...
def test_axes_same_shape2():
    w = generic_filter(
        jnp.asarray(_d), identity, 3, None, "constant", 0, 0, axes=(1, 2)
    )
...
def test_axes_mixedin_shape_and_value():
    u = generic_filter(
        jnp.asarray(_e), identity, 3, None, "constant", 0, 0, axes=(0, 2)
    )
    u2 = u[:, 0, :, 0]
    assert (
        numpy.all(u2 == _a)
